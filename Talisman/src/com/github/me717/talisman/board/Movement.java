...
public class Movement {
	...
	public static ArrayList<Space> getMovementOptions(PChar c, int roll,
			Board board) {
		...
		if (!c.isTeleport()) {
			...
			if (s.getRegion().equals(Region.OUTER)) {
				...
				if (remaining == 1) {
					...
					upRemaining = (upRemaining + Board.MIDDLE_REGION_SIZE)
							% Board.MIDDLE_REGION_SIZE;
					downRemaining = (downRemaining + Board.MIDDLE_REGION_SIZE)
							% Board.MIDDLE_REGION_SIZE;
					returning
							.add(board.getSpacesInRegion(Region.MIDDLE)[upRemaining]);
					returning
							.add(board.getSpacesInRegion(Region.MIDDLE)[downRemaining]);
				}
			}
		} else {
			returning.addAll(Arrays.asList(board.getSpacesInRegion(s.getRegion())));
		}
		return returning;
	}
}
